...
class CDLClip:
    ...
    def extract(self):
        AllCDLPaths=[]
        FileNames=[]
        for subdir,dirs,files in os.walk(CDLFolder):
            for file in files:
                if file.endswith(".tif"):
                    FileNames.append(file)
                    filepath = subdir+os.sep+file
                    AllCDLPaths.append(filepath)
        ClipFile = gpd.read_file(ClipPath) # read clipping shapefile to variable
        for count,value in enumerate(AllCDLPaths):
            ...
            if WriteClip==True:
                ...
                print("Wrote ",ClipTifName," to folder ",CDLClipOut)
            ...
            CDL_LOOKUP = pd.read_csv(LookupTablePath)
...
# ClipPath = Path(r'C:\Users\cjr2\Documents\GISData\DornCreek\GageQWatershed.shp')
# CDLFolder = Path(r'G:\NASS')
# OutFolder = Path(r'C:\Users\cjr2\Documents\GISData\DornCreek\CDL')      
# CDLClipFolder = Path(r'G:\NASS\ClipTemp')           
# LookupTablePath = Path(r'C:\Users\cjr2\Documents\GISData\LandCover\WI_CDL_2018\WI_CDL_2018\CDLCodeLookup.csv')           
# WriteClip=False      

# DornQCDL = CDLClip(ClipPath,"DornQ_05427927",CDLFolder,CDLClipFolder,OutFolder,LookupTablePath,WriteClip)
